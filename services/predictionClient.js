...
class PredictionClient {
  ...
  async healthCheck() {
    return new Promise((resolve, reject) => {
      ...
      const req = protocol.get(`${this.url}/health`, (res) => {
        let data = "";
      ...
      req.setTimeout(this.timeoutMs, () => {
        req.destroy(new Error(`Timeout API prediction après ${Math.round(this.timeoutMs / 1000)}s`));
      });
  ...
  async getFamilies() {
    return new Promise((resolve, reject) => {
      const protocol = this.url.startsWith("https") ? https : http;
      const req = protocol.get(`${this.url}/families`, (res) => {
        let data = "";
  ...
  async getLeagues(family) {
    return new Promise((resolve, reject) => {
      const protocol = this.url.startsWith("https") ? https : http;
      const req = protocol.get(`${this.url}/leagues/${family}`, (res) => {
        let data = "";
