class SuperAICouponGenerator {
  ...
  async sendMessage() {
    ...
    try {
      ...
      const fallback =
        message.includes("Reponse chat invalide") || message.includes("reponse vide")
          ? "L'assistant a repondu de maniere incomplete. Je relance le moteur dans un instant."
  ...
  saveConversationHistory() {
    try {
      localStorage.setItem("one_delux_super_ai_coupon_conversations", JSON.stringify(this.conversations.slice(-30)));
    } catch {
  ...
  loadConversationHistory() {
    try {
      const saved = localStorage.getItem("one_delux_super_ai_coupon_conversations");
