(function () {
  ...
  function announceVersion() {
    const key = "fury_one_browser_version_seen";
  ...
  function registerServiceWorker() {
    ...
    // Register service worker on all environments for FURY X ONE
  ...
  function setupVisibilitySync() {
    document.addEventListener("visibilitychange", () => {
      ...
      const evt = new CustomEvent("fury:resume-sync", { detail: { elapsedMs: elapsed } });
      window.dispatchEvent(evt);
    });

    window.addEventListener("fury:resume-sync", () => {
    ...
    window.addEventListener("fury:manual-update-check", async () => {
      if (!navigator.onLine) {
