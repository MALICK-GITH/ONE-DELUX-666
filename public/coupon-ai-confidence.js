...
class AIConfidenceCalculator {
  ...
  async loadHistoricalAccuracy() {
    try {
      ...
      const stored = localStorage.getItem('fury_one_historical_accuracy');
      if (stored) {
        const accuracy = JSON.parse(stored);
  ...
  extractMatchOdds(match) {
    const oddsElements = match.querySelectorAll('.premium-odd-value, .odd-box strong');
    const odds = [];
  ...
  extractMatchConfidence(match) {
    ...
    const confidenceElement = match.querySelector('.confidence-score, .prediction-main-confidence');
    if (confidenceElement) {
      const text = confidenceElement.textContent;
  ...
  calculateHistoricalFactor(match) {
    ...
    const teamHome = match.team1 || match.teamHome || 'Unknown';
    const teamAway = match.team2 || match.teamAway || 'Unknown';
  ...
  calculateRealTimeFactor(match) {
    // Calculate confidence based on实时 data
  ...
  extractMLFeatures(match) {
    ...
    return {
      odds: avgOdds,
      confidence: match.confiance || match.confidence || 50,
  ...
  getMinutesToStart(match) {
    ...
    const startTimeNum = typeof startTime === 'number' ? startTime : new Date(startTime).getTime() / 1000;
    return Math.max(0, (startTimeNum - now) / 60);
  }
