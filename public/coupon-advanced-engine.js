...
class AdvancedCouponEngine {
  ...
  async initializeMLModel() {
    try {
      // Use SiteAPI for predictions instead of separate ML models
      console.log('🧠 Using SiteAPI for ML predictions');
  ...
  async loadHistoricalData() {
    try {
      // Use localStorage for historical data
      const stored = localStorage.getItem('fury_one_historical_performance');
      if (stored) {
  ...
  updateOddsConfidence() {
    const liveMatches = document.querySelectorAll('[data-live="true"]');
    
    liveMatches.forEach(match => {
      const matchId = match.dataset.matchId || match.dataset.id;
  ...
  extractCurrentOdds(matchElement) {
    const oddsElements = matchElement.querySelectorAll('.premium-odd-value, .odd-box strong');
    const odds = [];
  ...
  async generateAdvancedCoupon(options = {}) {
    ...
    try {
      // Get available matches using SiteAPI
  ...
  calculateCorrelationRisk(match) {
    ...
    const odds = [match.odds?.home, match.odds?.away, match.odds?.draw].filter(o => o && o > 1);
  ...
  calculateRealTimeScore(match) {
    ...
    const booster = this.confidenceBoosters.get(match.matchId || match.id) || 0;
    score += booster;
  ...
  getAllCurrentOdds() {
    const odds = [];
    document.querySelectorAll('.premium-odd-value, .odd-box strong').forEach(el => {
      const odd = parseFloat(el.textContent);
      if (!isNaN(odd)) odds.push(odd);
    });
    return odds;
  }
  ...
  getRecentOddsMovements(matchId) {
    // Placeholder for odds movements
    return [];
  }

  async buildCorrelationMatrix(selections) {
  ...
  async findAlternativeMatch(match) {
    // Placeholder for alternative match finding
    return null;
  }

  validateTiming(selections) {
    // Validate timing of selections
    const issues = [];
    const now = Date.now();
    
    selections.forEach((selection, index) => {
      const startTime = selection.startTimeUnix * 1000;
      const minutesToStart = (startTime - now) / 60000;
      
      if (minutesToStart < 5) {
        issues.push(`Selection ${index + 1}: Too close to start time`);
      }
    });
  ...
  calculateTimingScore(selectedMatches) {
    // Calculate overall timing score
    const scores = selectedMatches.map(match => this.getTimingScore(match));
    return scores.reduce((a, b) => a + b, 0) / scores.length;
  }

  calculateStabilityScore(selectedMatches) {
    // Calculate overall stability score
    const scores = selectedMatches.map(match => match.stabilityScore || 50);
    return scores.reduce((a, b) => a + b, 0) / scores.length;
  }

  calculateMarketConditionScore(selectedMatches) {
    // Calculate market condition score
    const conditions = this.getCurrentMarketConditions();
    let score = 50;
    
    if (conditions.volatility < 0.02) score += 20;
    if (conditions.liquidity === 'high') score += 15;
    if (conditions.sentiment === 'bullish') score += 10;
    if (conditions.pressure === 'low') score += 15;
...
class BankrollOptimizer {
  ...
  kellyCriterion(coupon, bankroll) {
    ...
    const avgOdds = coupon.reduce((sum, pick) => sum + (pick.cote || pick.odds?.home || 1), 0) / coupon.length;
  ...
  calculateVolatility(coupon) {
    ...
    const odds = coupon.map(p => p.cote || p.odds?.home || 1);
    const avgOdds = odds.reduce((a, b) => a + b, 0) / odds.length;
    const variance = odds.reduce((sum, odd) => sum + Math.pow(odd - avgOdds, 2), 0) / odds.length;
